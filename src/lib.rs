...
/// org's declared conceal rules, flattened to the shape the host's compiler
/// takes: `(pattern, hide-groups, slot)`.
///
/// Exists so the conceal tests can live in the `integration` package without
/// naming a WIT-generated type across a crate boundary. They must run the
/// HOST's compiler rather than a re-implementation — a hand-rolled union here
/// could agree with itself forever while disagreeing with the thing that
/// renders the buffer — and the host crates are only available there.
#[doc(hidden)]
pub fn declared_conceal_rules() -> Vec<(String, Vec<u32>, Option<String>)> {
    conceal_rules()
        .into_iter()
