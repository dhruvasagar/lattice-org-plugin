...
// OA.18 — `the_span_chords_change_the_span_and_the_header_says_so` MOVED to
// `org_agenda_view_menu.rs`, as `the_span_rows_change_the_span_and_the_header_says_so`.
//
// It pressed `gDm` and `gDd`. Those chords are gone: `gD` is the view dispatch
// now, and a trie node cannot be both a terminal binding and a prefix. The
// journey it protected — same keystrokes, through the menu — is asserted there,
// where the harness declares the `transient-source` seam this one does not.
